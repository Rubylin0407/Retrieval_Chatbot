...
async function fetchQAHistory() {
    try {
        const response = await fetch('/api/v1/get-favorites');
        const user_data = await response.json();
            
        // Get a reference to the table element in your HTML
        const table = document.getElementById('fav-history-table');
        ...
        user_data.forEach(entry => {
            ...
            const QA_pair_id = entry.QA_pair_id;
            ...
            addButton.addEventListener('click', () => {
                ...
                RemoveFromFavorites(QA_pair_id, addButton);
            });
...
// Function to handle the "Remove from Favorites" action
function RemoveFromFavorites(QA_pair_id, button) {
